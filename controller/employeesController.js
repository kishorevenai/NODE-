...
const createEmployees = async(req,res) => {
     if(!req.body.firstname || !req.body.lastname) res.status(400).json({'message':'First and last names are required'})
     try {
...
const  updateEmployees = async (req,res) => {
    ...
    if(!employee) return res.status(400).json({'message':`No employee matches ID ${req.body.id}.`})
...
module.exports = { getAllEmployees,createEmployees,updateEmployees,deleteEmployees,getEmployee }